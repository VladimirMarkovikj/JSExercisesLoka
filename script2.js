// 1.Create a function forEach which takes an array and a callback,
...
// The array returned from eitherFilter should contain
// all elements in the passed-in array that yield a truthy
// return value when passed into EITHER of the two callbacks passed into eitherFilter.
...
// reduce(nums, add, 0); //-> 8
// Here's how it works:
// The function has an "accumulator value". Its job is to keep track of the accumulated output of each loop.
...
// Construct your own reduce function that accepts an array,
//  a callback, and an initial value and returns a single value.
// ADD CODE HERE
...
// 4. Construct a function intersection that compares input arrays and returns
...
const intersection = (...arrays) => {
  const allInOneArray = reduce(arrays[0], (a, b) => [...a, ...b], []);
  const resultArray = [];
  for (let i = 0; i < allInOneArray.length; i++) {
    let counter = allInOneArray.filter(
      (val) => val === allInOneArray[i]
    ).length;

    if (counter === arrays[0].length) {
      resultArray.push(allInOneArray[i]);
    }
  }
...
console.log(intersection([arr1, arr2, arr3])); // should log: [5, 15]
console.log("---------------------------------------------");
// 5. Construct a function union that takes an input array of arrays,
...
const union = (arrayOfArrays) => {
  const arrayResult = reduce(arrayOfArrays, (a, b) => [...a, ...b], []);
  return [...new Set(arrayResult)];
};

const arr51 = [5, 10, 15];
const arr52 = [15, 88, 1, 5, 7];
const arr53 = [100, 15, 10, 1, 5];
console.log(union([arr51, arr52, arr53])); // should log: [5, 10, 15, 88, 1, 7, 100]
console.log("---------------------------------------------");

// 6. Create a function groupBy that accepts an array and a callback,
...
const groupBy = (array, callback) => {
  const resultObj = {};
  for (let i = 0; i < array.length; i++) {
    if (typeof resultObj[callback(array[i])] === "undefined") {
      resultObj[callback(array[i])] = [array[i]];
    } else {
      resultObj[callback(array[i])] = [
        ...resultObj[callback(array[i])],
        array[i],
      ];
    }
...
const objOfMatches = (arrKeys, arrVals, callback) => {
  const resultObj = {};
  for (let i = 0; i < arrKeys.length; i++) {
    for (let j = 0; j < arrVals.length; j++) {
      if (callback(arrKeys[i]) === arrVals[j]) {
        resultObj[arrKeys[i]] = arrVals[j];
      }
    }
  }
  return resultObj;
...
const arr71 = ['hi', 'howdy', 'bye', 'later', 'hello'];
const arr72 = ['HI', 'Howdy', 'BYE', 'later', 'HELLO'];
function uppercaser(str) { return str.toUpperCase(); }
console.log(objOfMatches(arr71, arr72, uppercaser)); // should log: { hi: 'HI', bye: 'BYE', hello: 'HELLO' }
console.log("---------------------------------------------");
